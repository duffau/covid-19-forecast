...
def run(csv_file: str, output_folder: str):
    ...
    df.to_pickle(op.join(output_folder, 'population.pickle'))
    df.to_csv(op.join(output_folder, 'population.csv'))


def main():
    from glob import glob

    CSV_POP_PATTERN = '../data/raw/world_bank/population/API_SP.POP.TOTL*.csv'
